  ...
  <xsl:template match="uiConfig/items">
    <xsl:copy>
      <xsl:apply-templates select="node()|@*"/>
		...
		<!--<itemData guid="cb02648f-c474-4c65-8106-d41973939db7" noBmpOnMenu="true"
			   type="button" onInvoke='*Bind(DataSource=ToTextDataSource;Path=OnDraw)'
			   userCaption="Convert To Text"
			   enable='*Bind(DataSource=WDocCommandsDS;Path=CutEnable)'/>-->
		
      <!-- Define the button which shows the docker -->
      <itemData guid="a3481984-93a7-40b4-ab2c-c81df9f0407f" noBmpOnMenu="true"
                type="button"
                onInvoke='*Bind(DataSource=RenameDataSource;Path=CallRenameBox)'
                userCaption="Rename Object"
                enable="true"/>

      <!-- Define the web control which will be placed on our docker -->
      <itemData guid="8a30dae3-99e8-4a5c-bfdb-00ef9279e18e"
                type="wpfhost"
                hostedType="Addons\RenameMe\RenameMe.dll,RenameMe.RenameWarpercs"
                enable="true"/>
	...
	<xsl:template match="uiConfig/commandBars">
		<xsl:copy>
			...
			<container>
				...
				<item dock="fill" margin="0,0,0,0" guidRef="8a30dae3-99e8-4a5c-bfdb-00ef9279e18e"/>
			</container>
